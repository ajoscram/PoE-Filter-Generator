...
_INVALID_DOMAINS = [ "undefined", "map_device", "watchstone" ]
...
def get_bases(sieve: Sieve) -> set[str]:
    """Returns the list of base type names that match the `sieve` received."""
    bases = _get_bases(sieve, _get_pattern_for_bases)
...
def get_class_for_base(base_name: str) -> str:
    ...
    bases = _get_bases(Sieve(), lambda _: {})
...
def get_domains_and_tags(sieve: Sieve) -> tuple[set[str], set[str]]:
    ...
    tags = set()
    domains = set()
    for base_info in _get_bases(sieve, _get_pattern_for_tags).values():
        domains.add(base_info[DOMAIN_FIELD])
        tags.update(base_info[_TAGS_FIELD])
    return (domains, tags)

def _get_bases(sieve: Sieve, pattern_generator: Callable[[dict[str]], dict[str]]):
    bases: dict[str] = web.get(_URL, expiration=Expiration.MONTHLY, formatter=_format_bases_info)
...
def _is_base_info_valid(base_info: dict):
    ...
    valid_domain = base_info[DOMAIN_FIELD] not in _INVALID_DOMAINS

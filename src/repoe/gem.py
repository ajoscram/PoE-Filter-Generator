...
def is_base_gem(metadata_id: str):
    ...
    for gem_info in _get_gems().values():
        id_matches = metadata_id == gem_info[Field.BASE_ITEM.value][Field.ID.value]
        is_gem_base = gem_info[Field.DISPLAY_NAME.value] == gem_info[Field.BASE_ITEM.value][Field.DISPLAY_NAME.value]
...
def try_get_gem_base(name: str):
    ...
    return gems[name][Field.BASE_ITEM.value][Field.DISPLAY_NAME.value]
...
def _format_gems_info(gems_json: dict[str, dict]):
    return {
        gem_info[Field.DISPLAY_NAME.value]: gem_info
        for gem_info in gems_json.values()
        if _is_gem_info_valid(gem_info) }

def _is_gem_info_valid(gem_info: dict[str]):
    if Field.BASE_ITEM.value not in gem_info:
        return False

    if Field.DISPLAY_NAME.value not in gem_info:
        return False

    return Matchable(gem_info[Field.BASE_ITEM.value][Field.ID.value]) != ROYALE_PATTERN
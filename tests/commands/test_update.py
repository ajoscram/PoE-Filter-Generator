...
def test_execute_given_a_release_is_found_should_download_and_execute_the_updater(monkeypatch: MonkeyPatch):
    ...
    assert os_path_join_mock.received(_CURR_DIR, _UPDATER_EXE_NAME)
    assert web_download_mock.received(UPDATER_URL, _CURR_DIR, _UPDATER_EXE_NAME)
    ...
    assert _CURR_DIR in updater_params_string
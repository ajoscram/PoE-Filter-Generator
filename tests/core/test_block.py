from src.core.constants import SHOW, HIDE, CLASS, RULE_START, RULE_SEPARATOR
from src.core import constants
from src.core import Block

LINE_NUMBER = 1
DEFAULT_BLOCK_LINES = [ SHOW, f"Class == \"Currency\"", "BaseType == \"Jeweller's Orb\"" ]

def test_constructor_given_a_line_number_should_instantiate_correctly():
    block = Block(LINE_NUMBER)

    assert block.line_number == LINE_NUMBER
    assert block.lines == []
...
def test_extract_given_lines_that_represent_two_blocks_and_a_line_number_should_return_two_blocks():
    ...
    blocks = Block.extract(DEFAULT_BLOCK_LINES + ANOTHER_BLOCK)

    assert len(blocks) == 2
    assert len(blocks[0].lines) == len(DEFAULT_BLOCK_LINES)
    assert len(blocks[1].lines) == len(ANOTHER_BLOCK)

def test_comment_out_should_comment_out_lines_in_the_block():
    block = _create_block(DEFAULT_BLOCK_LINES)
...
def test_get_raw_lines_should_return_equivalent_raw_lines():
    ...
    for raw_line in DEFAULT_BLOCK_LINES:
...
def _create_block(lines: list[str]):
    return Block.extract(lines, LINE_NUMBER)[0]
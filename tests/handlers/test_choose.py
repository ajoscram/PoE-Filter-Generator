...
from core import ExpectedError, Block, Operand, Delimiter
...
def test_handle_given_more_than_one_choose_rule_in_block_should_raise():
    RULES = [ f"{Delimiter.RULE_SEPARATOR} {CHOOSE}", f"{Delimiter.RULE_SEPARATOR} {CHOOSE}" ]
    FILTER = create_filter(f"{Delimiter.COMMENT_START}{' '.join(RULES)}")
...
def test_handle_given_not_two_params_should_raise():
    PARAMS = [ "1", "2", "3" ]
    FILTER = create_filter(f"{Operand.SHOW} {Delimiter.RULE_START}{CHOOSE} {' '.join(PARAMS)}")
...
def test_handle_given_params_are_non_digit_(param_1: str, param_2: str, error_message: str):
    FILTER = create_filter(f"{Operand.SHOW} {Delimiter.RULE_START}{CHOOSE} {param_1} {param_2}")
...
def test_handle_given_subset_is_larger_than_set_should_raise():
    ...
    FILTER = create_filter(f"{Operand.SHOW} {Delimiter.RULE_START}{CHOOSE} {SET_SIZE} {SUBSET_SIZE}")
...
def test_handle_given_set_larger_than_number_of_lines_available_should_raise():
    SET_SIZE = 2
    FILTER = create_filter(f"{Operand.SHOW} {Delimiter.RULE_START}{CHOOSE} {SET_SIZE} {SET_SIZE - 1}")
...
def test_handle_given_combinable_lines_should_return_the_combinations():
    SET_SIZE = 3
    SUBSET_SIZE = SET_SIZE - 1
    FIRST_LINE = f"{Operand.SHOW} {Delimiter.RULE_START}{CHOOSE} {SET_SIZE} {SUBSET_SIZE}"

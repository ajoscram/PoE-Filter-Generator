...
def test_handle_given_a_blockname_should_import_the_appropriate_text(monkeypatch: MonkeyPatch):
    IMPORT_DESCRIPTION = f"{Splitter.BLOCKNAME} {_TARGET_BLOCK_NAME}"
    filter = create_filter(
    f"""{Operand.SHOW} {Delimiter.RULE_START}{RuleName.NAME} {_TARGET_BLOCK_NAME}
            {_TARGET_BLOCK_CONTENTS}
        {Operand.SHOW} {Delimiter.RULE_START}{RuleName.IMPORT} {IMPORT_DESCRIPTION}""")
...
def test_handle_given_a_root_should_resolve_to_the_directory_passed_via_options(
    ...
    target_filter = create_filter("target filter contents", filepath="target_filter")
    filter = create_filter(f"{Delimiter.RULE_START}{RuleName.IMPORT} {ROOT_NAME} {Splitter.ROOT} {target_filter.filepath}")
    filter_load_mock = FunctionMock(monkeypatch, Filter.load, target_filter, Filter)
    dirname_mock.result = "" # empty string means current filter's directory
...
def test_handle_given_import_file_doesnt_exist_should_raise(path_isfile_mock: FunctionMock):
    ...
    filter = create_filter(f"{Delimiter.RULE_START}{RuleName.IMPORT} {UNEXISTENT_FILEPATH}")
...
def test_handle_given_blockname_doesnt_exist_should_raise(monkeypatch: MonkeyPatch):
    ...
    filter = create_filter(f"{Delimiter.RULE_START}{RuleName.IMPORT} {Splitter.BLOCKNAME} {UNEXISTENT_BLOCKNAME}")
    _ = FunctionMock(monkeypatch, Filter.load, filter, Filter)
...
@pytest.mark.parametrize("rule_description, segment_splitter", [
    (f"navigation {Splitter.BLOCKNAME}", Splitter.BLOCKNAME),
    (f"navigation {Splitter.TEMPLATE}", Splitter.TEMPLATE),
])
def test_handle_given_an_empty_segment_that_shouldnt_should_raise(
    rule_description: str, segment_splitter: Splitter):
    
    filter = create_filter(f"{Delimiter.RULE_START}{RuleName.IMPORT} {rule_description}")
...
def test_handle_given_a_circular_reference_should_raise():
    BLOCK_NAME = "block"
    IMPORT_DESCRIPTION = f"{Splitter.BLOCKNAME} {BLOCK_NAME}"
    filter = create_filter(
    f"""{Operand.SHOW}  {Delimiter.RULE_START}{RuleName.NAME} {BLOCK_NAME}
        {Delimiter.RULE_START}{RuleName.IMPORT} {IMPORT_DESCRIPTION}""")
...
def test_handle_given_root_in_import_not_specified_through_options_should_raise():
    ...
    DESCRIPTION = f"{MISSING_ROOT} {Splitter.ROOT} file"
    filter = create_filter(
        f"{Delimiter.RULE_START}{RuleName.IMPORT} {DESCRIPTION}")
...
@pytest.mark.parametrize("splitter, segment_name", [
    (Splitter.ROOT, _NAVIGATION_SEGMENT.name),
    (Splitter.BLOCKNAME, _BLOCKNAME_SEGMENT.name),
    (Splitter.TEMPLATE, _TEMPLATE_SEGMENT.name) ])
def test_handle_given_too_many_splitters_in_import_should_raise(splitter: Splitter, segment_name: str):
    ERROR_PORTION = f"many {splitter} splitters"
    MISFORMATTED_DESCRIPTION = f"too {splitter} " + ERROR_PORTION
    filter = create_filter(
        f"{Delimiter.RULE_START}{RuleName.IMPORT} {MISFORMATTED_DESCRIPTION}")

import pytest
from core import ExpectedError, Operand, RULE_START
...
def test_handle_given_a_rule_without_a_description_should_raise():
    FILTER = create_filter(f"{Operand.SHOW} {RULE_START}{TAG}")
...
def test_handle_given_options_are_more_specific_than_the_rule_should_do_nothing():
    TAG_PATH = [ "1", "2", "3", "4" ]
    FILTER = create_filter(f"{Operand.HIDE} {RULE_START}{TAG} {' '.join(TAG_PATH[:-1])}")
...
def test_handle_given_rule_and_options_are_equivalent_should_show(option_tag: str, rule_tag: str):
    FILTER = create_filter(f"{Operand.HIDE} {RULE_START}{TAG} {rule_tag}")
...
def test_handle_given_rule_and_options_are_different_should_hide():
    FILTER = create_filter(f"{Operand.SHOW} {RULE_START}{TAG} {TAG}")

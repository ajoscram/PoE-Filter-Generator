...
def test_get_bases_given_a_range_should_return_if_in_it(
    web_get_mock: WebGetMock, value: int, is_in_range: bool):

    QUERY_TYPE = QueryType.CURRENCY # chosen arbitrarily
    web_get_mock.result = _DefaultExchangeResponse(value)

    base_types = ninja.get_bases(QUERY_TYPE, _LEAGUE_NAME, _DEFAULT_SIEVE, _RANGE)

    assert (_BASE_TYPE in base_types) == is_in_range

@pytest.mark.parametrize("replica", [ True, False, ])
def test_get_bases_given_replica_unique_should_return_depending_on_sieve(
...
def test_get_bases_given_unique_with_class_should_return_depending_on_sieve(
...
def test_get_bases_given_gem_with_level_should_return_depending_on_sieve(
...
def test_get_bases_given_gem_with_quality_should_return_depending_on_sieve(
...
def test_get_bases_given_corrupted_gem_should_return_depending_on_sieve(
...
def test_get_bases_given_item_level_for_wombgift_should_return_depending_on_sieve(
    web_get_mock: WebGetMock, level: bool):
    
    SIEVE = create_sieve_for_pattern({ Operand.ITEM_LEVEL: level })
    web_get_mock.result = _WombgiftResponse(level=0)

    base_types = ninja.get_bases(QueryType.WOMBGIFT, _LEAGUE_NAME, SIEVE, _RANGE)
